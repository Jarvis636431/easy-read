...
import { Range } from "~entries/popup/components/range"
import { Toggle } from "~entries/popup/components/toggle"
import { useAiReady } from "~entries/popup/hooks/use-ai-ready"
...
function IndexPopup() {
  ...
  const analyzeCurrentSiteWithAi = async () => {
    ...
    try {
      ...
      if (!("layout" in response)) throw new Error("AI 分析失败")
  ...
  const executeReadingCommand = async (instruction = readingCommand) => {
    ...
    try {
      ...
      if ("error" in response) throw new Error(response.error)
      if (!("layout" in response)) throw new Error("无法生成阅读计划")
  ...
  const aiReady = useAiReady(llmSettings)
